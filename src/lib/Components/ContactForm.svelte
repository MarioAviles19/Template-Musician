...
<div class="grid my-10 min-h-[25rem]">
    {#if success}
        <div in:fly={{delay: 400, duration: 500, x: "100%"}} out:fly={{duration: 500, x: "-100%"}} class="col-first row-first p-4 m-auto w-min min-w-clamp-sm shadow-2xl rounded-md bg-white">
    ...
    {:else}
    <form out:fly={{duration: 500, x: "-100%"}} in:fly={{delay: 400, duration: 500, x: "100%"}} onsubmit={preventDefault} class="col-first row-first m-auto w-min min-w-clamp-sm shadow-2xl p-4 rounded-md bg-white" {...restProps}>
        <h4 class="font-bold text-xl">Leave Me A Message</h4>
        <div class="flex gap-2 my-2 w-full justify-between">
            <input class="block rounded px-2 py-1" type="text" placeholder="First Name">
            <input class="block rounded px-2 py-1" type="text" placeholder="Last Name">
        </div>
        <input class="block w-full rounded px-2 py-1" type="email" placeholder="Email">
        <textarea class="w-full resize-none my-2 rounded" rows="5" placeholder="Message"></textarea>
