<script lang=ts>
    ...
    //out:fly={{duration: 500, x: "-100%"}} in:fly={{delay: 400, duration: 500, x: "100%"}}
    //in:fly={{delay: 400, duration: 500, x: "100%"}} out:fly={{duration: 500, x: "-100%"}} 
</script>
<div class="grid my-10 min-h-[25rem]">
    {#if success}
        <div in:fade={{delay: 500}} out:fade={{duration: 500}} class="col-first row-first p-4 m-auto w-full sm:max-w-clamp-sm shadow-2xl rounded-md bg-white">
    ...
    {:else}
    <form out:fade={{duration: 500}} in:fade={{delay: 500}}  onsubmit={preventDefault} class="col-first row-first m-auto w-full sm:max-w-clamp-sm shadow-2xl p-4 rounded-md bg-white" {...restProps}>
        ...
        <div class="text-right"><button class="px-2 py-1 bg-gradient-to-br bg-soft-black text-white rounded shadow-2xl text-lg">Submit</button></div>
    </form>
    {/if}
</div>
